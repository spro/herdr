...
`input` shows the inline input modal in the Herdr UI and prints the entered text to stdout once the user submits it with Enter. Esc cancels the prompt and the command exits with an error. The request is rejected with `modal_already_open` while another modal or input prompt is open.
